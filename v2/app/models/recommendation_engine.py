...
class RecommendationEngine:
    ...
    def get_hybrid_recommendations(self, user_id, num_recommendations=5, weights=None, context=None):
        """
        Generate hybrid recommendations combining collaborative filtering, content-based
        filtering, and contextual information
        """
        # If not using hybrid recommender, use standard user recommendations
        if not hasattr(self, 'hybrid_recommender'):
            return self.recommend_for_user(user_id, num_recommendations)
        
        # Get raw hybrid recommendations
        try:
            recommendations = self.hybrid_recommender.recommend(
                user_id=user_id,
                num_recommendations=num_recommendations * 2,  # Get more to allow for filtering
                weights=weights,
                context=context
            )
            
            # Apply financial behavior filtering if available
            if hasattr(self, 'financial_analyzer'):
                filtered_recommendations = self.financial_analyzer.filter_recommendations(
                    user_id=user_id,
                    recommendations=recommendations
                )
                
                # Get financial advice if available
                financial_advice = self.financial_analyzer.get_responsible_spending_advice(user_id)
                
                # Add financial advice to first recommendation if available
                if financial_advice and filtered_recommendations:
                    filtered_recommendations[0]['financial_advice'] = financial_advice
            ...
            # Fallback to standard recommendation if anything fails
            print(f"Error in hybrid recommendations: {str(e)}. Falling back to standard recommendations.")
            return self.recommend_for_user(user_id, num_recommendations)
